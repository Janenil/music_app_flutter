...
import '../../topsongPage/song.dart';
import '../data.dart';
class AlbumList extends StatefulWidget {
  final List<TopSongList> songList;
...
class _AlbumListState extends State<AlbumList> {
  ...
  Widget listViewDefault(List<TopSongList> list) {
    ...
    for (int i = 0; i < list.length; i++) {
      _list.add(
         new GestureDetector(
          ...
          onTap: () => Navigator.of(context).push(new MaterialPageRoute(builder: (BuildContext context,) => new SongPage('https${songinfo[i].purl.substring(4,songinfo[i].purl.length)}', list[i].data.songname, list[i]))),
           child:new Container(
              ...
              child: new Column(
                ...
                children: <Widget>[
                    ...
                    new Text(
                      list[i].data.singer[0].name +'·' +list[i].data.albumname,
                      maxLines: 1,
                      softWrap: false,
                      overflow: TextOverflow.ellipsis,
                      style: new TextStyle(
